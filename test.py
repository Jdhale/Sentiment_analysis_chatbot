...
class MarathiTextClassifier:
    ...
    def train_models(self, train_df, valid_df=None, test_df=None):
        ...
        # Vectorize text with optimized parameters for accuracy
        logger.info("Vectorizing text data...")
        self.vectorizer = TfidfVectorizer(
        ...
        # Define models optimized for text classification accuracy
        models = {
            ...
            'Naive Bayes': MultinomialNB(alpha=0.1),  # Smoothing parameter
        ...
        # Train individual models with time tracking
        logger.info("Training individual models...")
        for name, model in models.items():
            import time
            ...
            # For very large datasets, consider sampling for SVM
            if name == 'SVM' and len(X_train) > 10000:
                logger.info(f"Dataset large ({len(X_train)} samples), using subset for SVM training...")
            ...
            logger.info(f"{name} - Training time: {training_time:.2f}s, Accuracy: {accuracy:.4f}, F1: {f1:.4f}")
        
        # Train ensemble model with best performers
        logger.info("Training ensemble model...")
        ensemble = VotingClassifier([
            ('lr', models['Logistic Regression']),
